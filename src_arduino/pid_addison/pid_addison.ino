...
ros::Publisher left_speed("left_speed", &leftspeed); 
 
std_msgs::Float64 rightspeed;
ros::Publisher right_speed("right_speed", &rightspeed);
...
const int TICKS_PER_REVOLUTION = 442;
...
const double TICKS_PER_METER = 2165; // Originally 2880
...
void calc_vel_left_wheel(){
  ...
  leftspeed.data = float(velLeftWheel);
  left_speed.publish(&leftspeed);
...
void calc_vel_right_wheel(){
  ...
  rightspeed.data = velRightWheel;
  right_speed.publish(&rightspeed);
...
void setup() {
  ...
  nh.advertise(left_speed);
  nh.advertise(right_speed);
}
